...
- **账本服务**（[`src/index.ts`](src/index.ts)，`SessionCostService extends TypertRemoteService`）：在后台基于 `ctx.sessionQuery` 语料库对账——启动时先跑一轮预热，之后每个 `reconcileIntervalMs` tick（`ctx.interval`，随 fiber 释放）增量对账一轮——将每个会话的日志折叠越过其扫描水位、修剪已删除会话、隔离不可读日志。`cost.dashboard` 是对最近一次成功对账账本的纯读：打开看板或切换筛选从不等待语料扫描；失败的对账轮次保留旧数据可读并在下一轮重试；并发轮次永不重叠。行持久化在 `session_cost` 存储域中。
...
- 对账会重读每个事件越过其水位的会话日志（带受限的并发读取），每次看板调用都会在内存中物化行表；按日志指纹跳过未改动日志以及索引化汇总，延后到大语料部署需要时再做。`reconcileIntervalMs`（默认 5000）调节后台对账周期；schema 下限 1000 ms 用于防止误配成高频扫描。
- 崩溃一致性：启动快照只提供"有已写入水位覆盖"的持久化行，即恰好是最后一次已提交账本。进程在对账中途被杀可能留下部分重折的复用会话 id（新行配旧水位），在首轮对账检测到生命周期变化并重折之前可见；启动时区分它需要一次语料扫描，而快照设计刻意避免扫描。
- 浏览器缓存：切回已缓存的选择会立即重发其汇总并静默刷新；若切换与刷新之间恰逢北京日历翻期，旧周期的缓存值可能闪现一帧，随后被新汇总替换。
